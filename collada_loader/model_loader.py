...
class KeyFrame:
    ...
    def init_transform(self, joint_transform):
        for key, value in joint_transform.items():
            ...
            translation_matrix[0, 3], translation_matrix[1, 3], translation_matrix[2, 3] = value[0, 3], value[1, 3], value[2, 3]

            quaternion = quaternion_from_matrix(value - translation_matrix + np.identity(4))

            self.joint_transform[key] = [translation_matrix, quaternion]
...
class ColladaModel:
    def __init__(self, collada_file_path):
        ...
        anims = et.parse(collada_file_path).getroot().find("{http://www.collada.org/2005/11/COLLADASchema}library_animations").findall("{http://www.collada.org/2005/11/COLLADASchema}animation")
        self.__load_keyframes(anims)

        self.prev_iter = 0

    def __load_keyframes(self, anims):
        self.keyframes = []
        keyframes_times = [float(x) for x in anims[0].find("{http://www.collada.org/2005/11/COLLADASchema}source").find("{http://www.collada.org/2005/11/COLLADASchema}float_array").text.split(" ")]
        for index, time in enumerate(keyframes_times):
            joint_dict = dict()
            for animation in anims:
                joint_dict[animation.get("id")] = np.array([float(x) for x in animation.findall("{http://www.collada.org/2005/11/COLLADASchema}source")[1].find("{http://www.collada.org/2005/11/COLLADASchema}float_array").text.split(" ")])[index * 16:(index + 1) * 16].reshape((4, 4))
    ...
    def animation(self, shader_program, speed = 1.0):
        ...
        while (self.keyframes[self.prev_iter].time > current_time) or (self.keyframes[self.prev_iter + 1].time < current_time):
            self.prev_iter = (self.prev_iter + 1)%(len(self.keyframes) - 1)

        
        duration = self.keyframes[self.prev_iter+1].time - self.keyframes[self.prev_iter].time
        frame_progress = (current_time - self.keyframes[self.prev_iter].time)/(duration)
